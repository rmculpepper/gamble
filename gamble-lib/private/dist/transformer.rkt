...
(define-dist-struct affine-distx
  ...
  [(define (-cdf self y log? 1-p?)
   ...
   (define (-mean self)
     (match-define (affine-distx d a b) self)
     (let ([m (dist-mean d)]) (and m (affine-apply a b m))))
   (define (-median self)
     (match-define (affine-distx d a b) self)
     (let ([m (dist-median d)]) (and m (affine-apply a b m))))
   (define (-modes self)
     (match-define (affine-distx d a b) self)
     (let ([ms (dist-modes d)]) (and ms (map (lambda (m) (affine-apply a b m)) ms))))
   (define (-variance self)
     (match-define (affine-distx d a b) self)
     (let ([var (dist-variance d)]) (and var (* var a a))))])
...
(define-dist-struct truncate-distx
  ...
  #:guard (lambda (dist a b)
            (unless (< a b)
              (error 'truncate-distx "empty range\n  range: (~e, ~e)" a b))
  ...
  #:methods gen:dist
  [(define (-sample self)
     (match-define (truncate-distx d a b) self)
