...
(define (db-count* h [ok-tag? #f])
  (if ok-tag?
      (for/sum ([k (in-hash-keys h)] #:when (ok-tag? (dbkey-tag k))) 1)
      (hash-count h)))
...
(define tracing-stochastic-ctx%
  (class base-stochastic-ctx%
    ...
    (define/override (-sample dist tag addr)
      (unless (or tag addr)
        (error 'sample "either unique tag or address is required for MCMC sampler~a\n  dist: ~e"
               ";\n address management failed because of uninstrumented code"
               dist))
      (define key (dbkey tag addr))
      (when (hash-ref current-db key #f)
        (error 'sample "duplicate key\n  tag: ~e\n  address: ~e" tag addr))
      (define delta-e (hash-ref delta-db key #f))
      (define prev-e (hash-ref prev-db key #f))
      (cond [delta-e (sample/delta dist key delta-e prev-e)]
            [prev-e (sample/prev dist key prev-e)]
            [else (sample/new dist key #f)]))

    (define/private (sample/delta dist key delta-e prev-e)
      (unless prev-e (error 'sample "internal error: in delta, not in previous"))
      (cond [(entry? delta-e)
             (log-mcmc-info "DELTA ~s: ~e, ~e => ~e, ~e" key
      ...
      (cond [(equal? (entry-dist prev-e) dist)
            ...
            [(eq? (dist-type (entry-dist prev-e)) (dist-type dist))
             ...
             (cond [(logspace-nonzero? new-lpr)
                    ...
                    (log-mcmc-info "RESCORE ~s: ~e, ~e" key dist value)
                    (db-add! key new-e prev-e)
                    value]
                   [else (fail 'sample-rescore)])]
            [else (sample/new dist key prev-e)]))

    (define/private (sample/new dist key prev-e)
      ...
      (if prev-e
          (log-mcmc-info "MISMATCH ~s: ~e, ~e => ~e, ~e" key
                         (entry-dist prev-e) (entry-value prev-e)
                         dist value)
          (log-mcmc-info "NEW ~s: ~e, ~e" key dist value))
      (db-add! key (entry dist value lpr) prev-e)
      value)
    ...
    ;; db-add! : Key Entry (U #f Entry) -> Void
  ...
  (class tracing-stochastic-ctx%
    ...
    (define/override (-sample dist tag addr)
      (match (get-value tag dist)
        [(list value)
         (define lpr (dist-pdf dist value #t))
         (hash-set! prev-db (dbkey tag addr) (entry dist value lpr))]
        [_ (void)])
